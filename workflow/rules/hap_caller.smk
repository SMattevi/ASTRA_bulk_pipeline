...
rule HardFiltering:
    input:
        "results_{sample_id}/{tec}/haplotypeCaller/{tec}.vcf.gz"
    output:
        final="results_{sample_id}/{tec}/filtration/filtered.vcf.gz",
        ind_in="results_{sample_id}/{tec}/filtration/indels.vcf.gz",
        snps_in="results_{sample_id}/{tec}/filtration/snps.vcf.gz",
        ind_fin="results_{sample_id}/{tec}/filtration/indels_filtered.vcf.gz",
        snps_fin="results_{sample_id}/{tec}/filtration/snps_filtered.vcf.gz"
    conda: "../envs/gatk.yml"
    params: 
    ...
    shell:
        ...
        gatk CollectVariantCallingMetrics -I {output.final} --DBSNP {params.dbsnp} -SD {params.dict_gen} -O results_{params.sampleid}/{wildcards.tec}/filtration/metrics """
...
rule het_selection_rna:
    ...
    shell:
        ...
        bcftools reheader -s {params.samplefile} {output.noed} -o {output.fin}        
        tabix {output.fin} """
